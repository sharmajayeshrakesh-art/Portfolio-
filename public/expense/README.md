...
Pay for something on Google Pay, tap Share on the receipt, pick Kharcha from the
Android share sheet. It reads the screenshot on the device, shows what it found,
and you tap Save.
...
there is no server copy, **so CSV export is the backup strategy, not a
convenience.** The app nags if the last export is over 30 days old.

Import accepts its own export and is tolerant enough for a bank statement:
columns are matched by header name, several date formats are accepted, and rows
that are credits or unparseable are counted and reported rather than dropped
silently.
...
src/store.js       localStorage, dates, totals
...
src/app.js         screens, routing, share → review → save
...
7. Settings → Export, and confirm the CSV opens in a spreadsheet.
