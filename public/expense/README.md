...
pick Kharcha from the Android share sheet — it reads the screenshot on the
...
2. `sw.js` intercepts `POST` requests ending in `/share`, pulls the image out of
   the `FormData`, stores it in the `kharcha-share` cache, and replies with a
...
`localStorage` under `kharcha.v1`. A browser can clear it without warning and
...
icons/             generated by scripts/gen-kharcha-icons.mjs
```

## Renaming

`APP_NAME` in `src/base.js`, plus `name`/`short_name` in `manifest.json` and the
`<title>`. Changing the storage key in `src/store.js` would orphan existing data.
...
3. Chrome menu → **Add to home screen**. It should open fullscreen with no
   browser chrome.
4. Open Google Pay, any receipt, **Share** → Kharcha should be in the sheet.
